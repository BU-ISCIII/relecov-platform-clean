...
def mutations_in_lineage(request):
    ...
    relecov_dashboard.utils.var_needle_mutation_graph_by_lineage.create_needle_plot_graph_mutation_by_lineage(
...
def spike_mutations_3d(request):
    relecov_dashboard.utils.var_molecule3D_bn_graph.create_model3D_bn()
...
def lineages_voc(request):
    ...
    draw_lineages["lineage_on_time"] = (
        relecov_dashboard.utils.var_lineage_variation_over_time_graph.create_lineages_variations_graphic()
    )
    return render(
        request,
...
def samples_received_over_time_pie(request):
    ...
    relecov_dashboard.utils.var_samples_received_over_time_pie.create_samples_received_over_time_per_laboratory_pieChart(
        data
    )
...
def methodology_sample_processing(request):
    sample_processing = (
        relecov_dashboard.utils.met_sample_preprocessing.sample_processing_graphics()
    )
...
def methodology_bioinfo(request):
    bioinfo = relecov_dashboard.utils.met_bioinfo.bioinfo_graphics()
    return render(

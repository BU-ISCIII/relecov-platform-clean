...
def get_all_chromosome_objs():
    ...
    if relecov_core.models.Chromosome.objects.all().exists():
        return relecov_core.models.Chromosome.objects.all()
    return None
...
def get_all_organism_objs():
    ...
    if relecov_core.models.OrganismAnnotation.objects.all().exists():
        return relecov_core.models.OrganismAnnotation.objects.all()
    return None
...
def get_default_chromosome():
    ...
    """
    if relecov_core.models.Chromosome.objects.all().exists():
        return relecov_core.models.Chromosome.objects.order_by("created_at").first()
    else:
        return None
...
def get_sample_in_variant_list(chromosome_obj):
    ...
    if relecov_core.models.VariantInSample.objects.filter(
        ...
        v_in_sample_objs = relecov_core.models.VariantInSample.objects.filter(
            variantID_id__chromosomeID_id=chromosome_obj
...
def get_variant_graphic_from_sample(sample_id):
    ...
    return relecov_core.utils.plotly_graphics.needle_plot(v_data)
...
def get_gene_list(chromosome_obj):
    ...
    if relecov_core.models.Gene.objects.filter(chromosomeID=chromosome_obj).exists():
        gene_objs = relecov_core.models.Gene.objects.filter(chromosomeID=chromosome_obj)

...
def index(request):
    ...
    nextstrain_url = relecov_core.utils.generic_functions.get_configuration_value("NEXTSTRAIN_URL")
    return render(
        request,
...
def sample_display(request, sample_id):
    sample_data = relecov_core.utils.samples.get_sample_display_data(sample_id, request.user)
    if "ERROR" in sample_data:
        return render(
            request, "relecov_core/sampleDisplay.html", {"ERROR": sample_data["ERROR"]}
        )
    sample_data["gisaid"] = relecov_core.utils.public_db.get_public_information_from_sample("gisaid", sample_id)
    sample_data["ena"] = relecov_core.utils.public_db.get_public_information_from_sample("ena", sample_id)
    sample_data["bioinfo"] = relecov_core.utils.bioinfo_analysis.get_bioinfo_analysis_data_from_sample(sample_id)
    sample_data["lineage"] = relecov_core.utils.lineage.get_lineage_data_from_sample(sample_id)
    sample_data["variant"] = relecov_core.utils.variants.get_variant_data_from_sample(sample_id)
    # Display graphic only if variant data are for the sample
    if "heading" in sample_data["variant"]:
        sample_data["graphic"] = relecov_core.utils.variants.get_variant_graphic_from_sample(sample_id)
    return render(
...
def search_sample(request):
    ...
    if request.method == "POST" and request.POST["action"] == "searchSample":
        ...
        if s_date != "" and not relecov_core.utils.generic_functions.check_valid_date_format(s_date):
            return render(
                request,
...
def metadata_visualization(request):
    ...
    if request.method == "POST" and request.POST["action"] == "selectFields":
        selected_fields = relecov_core.utils.schema_handling.store_fields_metadata_visualization(request.POST)
        if "ERROR" in selected_fields:
            m_visualization = relecov_core.utils.schema_handling.get_fields_from_schema(
                relecov_core.utils.schema_handling.get_schema_obj_from_id(request.POST["schemaID"])
            )
            return render(
                request,
    ...
    metadata_obj = relecov_core.utils.schema_handling.get_latest_schema("Relecov", __package__)
    ...
    data_visualization = relecov_core.utils.schema_handling.fetch_info_meta_visualization(metadata_obj)
    ...
    m_visualization = relecov_core.utils.schema_handling.get_fields_from_schema(metadata_obj)
    return render(
        request,
        "relecov_core/metadataVisualization.html",
...
def intranet(request):
    ...
    if relecov_group not in request.user.groups.all():
        ...
        date_lab_samples = relecov_core.utils.samples.get_sample_per_date_per_lab(lab_name)
        if len(date_lab_samples) > 0:
            sample_lab_objs = relecov_core.utils.samples.get_sample_objs_per_lab(lab_name)
            analysis_percent = relecov_core.utils.labs.get_bio_analysis_stats_from_lab(lab_name)
            cust_data = {
            ...
            intra_data["sample_bar_graph"] = relecov_core.utils.samples.create_date_sample_bar(
                date_lab_samples, cust_data
            )
            intra_data["sample_gauge_graph"] = relecov_core.utils.samples.perc_gauge_graphic(
                analysis_percent
            )
            intra_data["actions"] = relecov_core.utils.samples.get_lab_last_actions(lab_name)
            gisaid_acc = relecov_core.utils.public_db.get_public_accession_from_sample_lab(
                "gisaid_accession_id", sample_lab_objs
            )
            if len(gisaid_acc) > 0:
                intra_data["gisaid_accession"] = gisaid_acc
            intra_data["gisaid_graph"] = relecov_core.utils.public_db.percentage_graphic(
            ...
            if len(ena_acc) > 0:
                ...
                intra_data["ena_graph"] = relecov_core.utils.public_db.percentage_graphic(
        ...
        manager_intra_data = {}
        all_sample_per_date = relecov_core.utils.samples.get_sample_per_date_per_all_lab()
        ...
        if len(all_sample_per_date) > 0:
            ...
            manager_intra_data["sample_bar_graph"] = relecov_core.utils.samples.create_date_sample_bar(
            ...
            manager_intra_data["sample_gauge_graph"] = relecov_core.utils.samples.perc_gauge_graphic(
                analysis_percent
            )
            # dash graph for samples per lab
            relecov_core.utils.samples.create_dash_bar_for_each_lab()
            # Get the latest action from each lab
            manager_intra_data["actions"] = relecov_core.utils.samples.get_lab_last_actions()
            ...
            if len(gisaid_acc) > 0:
                ...
                manager_intra_data["gisaid_graph"] = relecov_core.utils.public_db.percentage_graphic(
                    num_of_samples["Defined"], len(gisaid_acc), ""
                )
            # Collect Ena information
            ena_acc = relecov_core.utils.public_db.get_public_accession_from_sample_lab("ena_sample_accession", None)
            if len(ena_acc) > 0:
                manager_intra_data["ena_accession"] = ena_acc
                manager_intra_data["ena_graph"] = relecov_core.utils.public_db.percentage_graphic(
...
def metadata_form(request):
    schema_obj = relecov_core.utils.schema_handling.get_latest_schema("relecov", __package__)
    ...
    if request.method == "POST" and request.POST["action"] == "defineSamples":
        ...
        if len(res_analyze) == 0:
            m_form = relecov_core.utils.samples.create_metadata_form(schema_obj, request.user)
            return render(request, "relecov_core/metadataForm.html", {"m_form": m_form})
        if "save_samples" in res_analyze:
            s_saved = relecov_core.utils.samples.save_temp_sample_data(res_analyze["save_samples"], request.user)
        ...
        m_batch_form = relecov_core.utils.samples.create_form_for_batch(schema_obj, request.user)
    ...
    if request.method == "POST" and request.POST["action"] == "defineBatch":
        ...
        meta_data = relecov_core.utils.samples.join_sample_and_batch(request.POST, request.user, schema_obj)
...
def organism_annotation(request):
    ...
    if request.method == "POST" and request.POST["action"] == "uploadAnnotation":
        gff_parsed = relecov_core.utils.annotation.read_gff_file(request.FILES["gffFile"])
...
def received_samples(request):
    ...
    sample_data["received_samples_graph"] = relecov_core.utils.samples_graphics.display_received_samples_graph()
    # Pie charts
    # data = parse_json_file()
    # create_samples_received_over_time_per_ccaa_pieChart(data)
    sample_data["samples_per_ccaa"] = relecov_core.utils.samples_graphics.display_received_per_ccaa()
    # create_samples_received_over_time_per_laboratory_pieChart(data)
    sample_data["samples_per_lab"] = relecov_core.utils.samples_graphics.display_received_per_lab()
    return render(
        request,
